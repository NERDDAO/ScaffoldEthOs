...
const TokenInputRegular = ({ type, onNormalChange, onChange, placeholder, removeBalance, removeMax, tokenImage, name, onElement, onlySelections, tokens, tokenOnly, noETH, selected, outputValue, disabled, noBalance, onImageClick }) => {
    ...
    return modalIsOpen ? (
        ...
        <React.Fragment>
            {
                !removeBalance &&
                <span className="BalanceImputPRCD">
                    {
                        !noBalance && !tokenOnly && element && balance === null
                            ? <React.Fragment>Loading...</React.Fragment>
                            : (element && balance !== null && !noBalance && !tokenOnly)
                                ? <React.Fragment>Balance: {formatMoney(fromDecimals(balance, element.decimals, true))} {shortenWord({ context, charsAmount: 15 }, element.symbol)}</React.Fragment>
                                : <React.Fragment>Balance: 0</React.Fragment>
                    }
                </span>
            }
            <div className={`form-control`}>
                ...
                <input
                    ...
                    step="0.001"
                    value={valueInput}
                    onChange={onNormalChange ? onChange : onValueChange}
                />
                {
                    !removeMax &&
                    <button
                        onClick={() => !disabled && setValue(fromDecimals(balance, element.decimals, true))}
