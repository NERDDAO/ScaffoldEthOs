...
export async function getAMMs({ context, chainId, newContract }) {
  var ammAggregatorAddress = getNetworkElement({ context, chainId }, "ammAggregatorAddress");
  var ammAggregator = newContract(context.AMMAggregatorABI, ammAggregatorAddress);
  var amms = await Promise.all(
    (
      await blockchainCall(ammAggregator.methods.amms)
    ).map(async address => {
      ...
      return {
        ...amm,
        ...amm.data,
        ethereumAddress: amm.data[0],
        name: amm.info[0],
        version: amm.info[1],
        image: `${process.env.PUBLIC_URL}/img/amms/${amm.info[0]}.png`,
      };
    }),
  );
...
async function calculateUniswapV3PriceWithSlippage(
  { provider, context, chainId },
  pool,
  inputTokenAddress,
  outputTokenAddress,
  middleTokenAddress,
  ...
  var path =
    inputTokenAddress + conversionEncode[pool[0].fee] + (middleTokenAddress || outputTokenAddress).substring(2);
  ...
  if (method === "quoteExactOutput") {
    ...
    if (middleTokenAddress) {
      swapPath =
        outputTokenAddress +
        conversionEncode[pool[1].fee] +
        middleTokenAddress.substring(2) +
        conversionEncode[pool[0].fee] +
        inputTokenAddress.substring(2);
    }
  }

  var output;
  try {
    output = await sendAsync(
      provider,
      "eth_call",
      {
        to: getNetworkElement({ context, chainId }, "uniswapV3QuoterAddress"),
        data:
          web3Utils.sha3((method || "quoteExactInput") + "(bytes,uint256)").substring(0, 10) +
          abi.encode(["bytes", "uint256"], [swapPath, value]).substring(2),
      },
      "latest",
    );
  } catch (e) {
  ...
  var tokens = [inputTokenAddress, middleTokenAddress || outputTokenAddress];
      ...
      .decode(
        ...
        await sendAsync(
          ...
          "eth_call",
          {
            to: tokens[0],
            data:
              web3Utils.sha3("balanceOf(address)").substring(0, 10) +
              abi.encode(["address"], [pool[0].liquidityPoolAddress]).substring(2),
          },
          "latest",
        ),
      )[0]
...
export async function elaborateForUniswapV3Calculation(
  { context, chainId },
  input,
  output,
  amm,
  ...
  const outputTokenAddress =
    output.token.address === VOID_ETHEREUM_ADDRESS ? amm.ethereumAddress : output.token.address;
  ...
  var pools = await Promise.all(
    Object.keys(conversionEncode).map(async fee => ({
      fee,
      liquidityPoolAddress: abi.decode(
        ["address"],
        await sendAsync(
          provider,
          "eth_call",
          {
            to: factoryAddress,
            data:
              web3Utils.sha3("getPool(address,address,uint24)").substring(0, 10) +
              abi
                .encode(
                  ["address", "address", "uint24"],
                  [inputTokenAddress, middleTokenAddress || outputTokenAddress, fee],
                )
                .substring(2),
          },
          "latest",
  ...
  if (middleTokenAddress) {
    var otherPools = await Promise.all(
      Object.keys(conversionEncode).map(async fee => ({
        fee,
        liquidityPoolAddress: abi.decode(
          ["address"],
          await sendAsync(
            ...
            "eth_call",
            {
              ...
              data:
                web3Utils.sha3("getPool(address,address,uint24)").substring(0, 10) +
                abi
                  .encode(["address", "address", "uint24"], [middleTokenAddress, outputTokenAddress, fee])
                  .substring(2),
            },
            "latest",
          ),
  ...
  var decimals = [
    ...
    abi
      .decode(["uint256"], await getRawField({ provider }, middleTokenAddress || outputTokenAddress, "decimals"))[0]
      .toString(),
  ];

  const prices = await Promise.all(
    pools.map(pool =>
      calculateUniswapV3PriceWithSlippage(
        { provider, context, chainId },
        pool,
        inputTokenAddress,
        outputTokenAddress,
...
export async function calculateUniswapV3SwapOutput({ context, chainId }, input, output, amm, middleTokenAddress) {
  const prices = await elaborateForUniswapV3Calculation(
    { context, chainId },
    input,
    output,
    amm,
...
async function calculateAMMBasedSwapInput(
  data,
  input,
  output,
  amm,
  ...
  var path = [inputTokenAddress, ...inputTokens];

  if (amm.name === "UniswapV2") {
    var result = (
      await blockchainCall(
        newContract(context.uniswapV2RouterABI, context.uniswapV2RouterAddress).methods.getAmountsIn,
        output.value,
        path,
      )
    )
  ...
  if (amm.name === "SushiSwap") {
    var result = (
      await blockchainCall(
        newContract(context.uniswapV2RouterABI, context.sushiSwapRouterAddress).methods.getAmountsIn,
        output.value,
        path,
      )
    )
...
async function calculateAMMBasedSwap(data, input, output, amm, middleTokenAddress, exactOutput) {
  ...
  var liquidityPoolAddresses = [
    (
      await blockchainCall(amm.contract.methods.byTokens, [inputTokenAddress, middleTokenAddress || outputTokenAddress])
    )[2],
  ];

  if (middleTokenAddress) {
    inputTokens.push(middleTokenAddress);
    liquidityPoolAddresses.push(
      (await blockchainCall(amm.contract.methods.byTokens, [middleTokenAddress, outputTokenAddress]))[2],
    );
  }
  ...
  var dataArray = exactOutput
    ? await calculateAMMBasedSwapInput(data, input, output, amm, liquidityPoolAddresses, inputTokenAddress, inputTokens)
    : await blockchainCall(
        amm.contract.methods.getSwapOutput,
        inputTokenAddress,
        input.value,
        liquidityPoolAddresses,
        inputTokens,
  ...
  var liquidityPoolAddress = await blockchainCall(
    amm.contract.methods.byLiquidityPool,
  ...
  var token0Decimals =
    tokens[0] === VOID_ETHEREUM_ADDRESS
      ? "18"
      : abi.decode(["uint256"], await getRawField({ provider }, tokens[0], "decimals"))[0].toString();
  var token1Decimals =
    tokens[1] === VOID_ETHEREUM_ADDRESS
      ? "18"
      : abi.decode(["uint256"], await getRawField({ provider }, tokens[1], "decimals"))[0].toString();

  var tokenDecimals = [token0Decimals, token1Decimals];

  var normalizedInputAmount = parseFloat(
    fromDecimals(dataArray[dataArray.length - 2], tokenDecimals[inputPosition], true),
  );
...
export async function calculateSwapOutput({ chainId, context }, input, output, amm) {
  ...
  const inputTokenAddress = web3Utils.toChecksumAddress(
    input.token.address === VOID_ETHEREUM_ADDRESS ? amm.ethereumAddress : input.token.address,
  );
  const outputTokenAddress = web3Utils.toChecksumAddress(
    output.token.address === VOID_ETHEREUM_ADDRESS ? amm.ethereumAddress : output.token.address,
  );

  const middleTokens =
    amm.name === "Mooniswap" || amm.name === "Balancer"
      ? [undefined]
      : [
          undefined,
          amm.ethereumAddress,
  ...
  var data = await Promise.all(
    middleTokens.map(async middleTokenAddress => {
      var swapOutput = "0";
      var priceImpact = "0";
      var liquidityPoolAddresses;
      var path;
  ...
  selected.middleSymbol =
    selected.middleTokenAddress &&
    abi.decode(
      ["string"],
      await getRawField({ provider: amm.contract.currentProvider }, selected.middleTokenAddress, "symbol"),
    )[0];
...
const swapActions = {
  UniswapV2BasedSwap(address, data, swapData, amountOutMinimum, value) {
    ...
    const args = [newContract(context.uniswapV2RouterABI, address).methods[methodName]];
  ...
  Balancer(data, swapData, amountOutMinimum, value) {
    ...
    return blockchainCall(
      contract.methods.swapExactAmountIn,
      swapData.inputToken,
      swapData.amount,
      swapData.path[0],
      amountOutMinimum,
...
export async function performSwap(data, input, output, amm, swapDataInfo, slippage, receiver) {
  ...
  if (amm.name === "UniswapV3") {
    await performUniswapV3Swap(
      { chainId, context, account },
      input,
      output,
      amm,
      swapDataInfo,
...
export async function performUniswapV3Swap(
  ...
  output.token.address === VOID_ETHEREUM_ADDRESS &&
    multicallData.push(amm.contract.methods.unwrapWETH9(0, realReceiver).encodeABI());
...
export async function calculateSwapInput(data, input, output, amm) {
  ...
  var selected = Math.min.apply(
    null,
    swapInputs.filter(it => it > 0),
  );
  selected = swapInputs.indexOf(selected);
  selected = data[selected];
  selected.middleSymbol =
