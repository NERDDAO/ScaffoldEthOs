...
import { tickToPrice, nearestUsableTick, TICK_SPACINGS, TickMath } from "@uniswap/v3-sdk/dist/";
...
async function getFactory(data, generation) {
  ...
  var factoryAddress = (await blockchainCall(getGlobalContract("factoryOfFactories").methods.get, factoryIndex))
    .factoryList;
...
export async function allFarmings(data, factoryAddress, generation) {
  ...
  if (!generation) {
    return (
      await Promise.all(
        ...
        ].map(it => allFarmings(data, it[0], it[1])),
      )
    ).reduce((all, it) => [...all, ...it], []);
  }

  factoryAddress =
    factoryAddress ||
    getNetworkElement({ context, chainId }, generation === "gen2" ? "farmGen2FactoryAddress" : "farmFactoryAddress");
  ...
  var farmingContractAddresses = (await getLogs(web3.currentProvider, "eth_getLogs", args)).map(it =>
  ...
  if (rewardTokenAddress) {
    ...
    args.topics = [web3Utils.sha3("RewardToken(address)"), abi.encode(["address"], [rewardTokenAddress])];
  ...
  if (mode === "positions") {
    const events = await getLogs(web3.currentProvider, "eth_getLogs", {
      ...
      topics: [web3Utils.sha3("Transfer(uint256,address,address)"), [], [], abi.encode(["address"], [account])],
      fromBlock: web3Utils.toHex(getNetworkElement({ context, chainId }, "deploySearchStart") || "0x0"),
      toBlock: "latest",
    });
    events.forEach(it =>
      (positionIds[it.address] = positionIds[it.address] || []).push(
        abi.decode(["uint256"], it.topics[1])[0].toString(),
      ),
    );
    farmingContractAddresses = Object.keys(positionIds);
  }

  var farmingContracts = await Promise.all(
    farmingContractAddresses.map(it => getFarming({ ...data, positionIds: positionIds[it] }, it, generation)),
  );
...
export async function getFarming(data, address, generation) {
  ...
  try {
    farmTokenCollection = newContract(
      context.INativeV1ABI,
      await blockchainCall(contract.methods._farmTokenCollection),
    );
  } catch (e) {}
  const extensionAddress = await blockchainCall(contract.methods.host);
  const extensionContract = newContract(
    context[generation === "gen2" ? "FarmExtensionGen2ABI" : "FarmExtensionGen1ABI"],
    extensionAddress,
  );
  ...
  await Promise.all(
    setups.map(async (setup, i) => {
      ...
      setup.canActivateSetup =
        parseInt(setupInfo.renewTimes) > 0 && !setup.active && parseInt(setupInfo.lastSetupIndex) === parseInt(i);
      if (!byMint && setup.rewardPerBlock !== "0") {
        setup.canActivateSetup =
          setup.canActivateSetup &&
          parseInt(extensionBalance) >= parseInt(setup.rewardPerBlock) * parseInt(setupInfo.blockDuration);
      }
  ...
  if (positionIds && positionIds.length > 0) {
    for (const positionId of positionIds) {
      ...
      try {
        ...
        setup = {
          ...
          setupInfo,
          free: setupInfo.free || generation === "gen2",
          generation,
        };
        isValidPosition(data, position) &&
          positions.push({ positionId, ...position, ...setup, setup, setupInfo, contract, generation });
      } catch (e) {
        console.log(e);
      }
    }
    if (farmTokenCollection) {
      const args = {
        ...
        topics: [web3Utils.sha3("FarmToken(uint256,address,uint256,uint256)")],
        fromBlock: web3Utils.toHex(getNetworkElement({ context, chainId }, "deploySearchStart") || "0x0"),
        toBlock: "latest",
      };
  ...
  try {
    isRegular =
      generation === "gen2" &&
      (await getFactory(data)).indexOf(await blockchainCall(contract.methods.initializer)) !== -1;
  } catch (e) {}
...
export async function getFarmingContractGenerationByAddress(data, address) {
  ...
  const args = {
    ...
    topics: [web3Utils.sha3("Deployed(address,address,address,bytes)"), [], abi.encode(["address"], [address])],
    fromBlock: web3Utils.toHex(getNetworkElement({ context, chainId }, "deploySearchStart")) || "0x0",
    toBlock: "latest",
  };
...
export async function loadFarmingSetup(data, contract, i) {
  ...
  var models = {
    setup: {
      types: ["uint256", "bool", "uint256", "uint256", "uint256", "uint256", "uint256", "uint256"],
      names: [
        "infoIndex",
        "active",
        "startBlock",
  ...
  var types = [`tuple(${models.setup.types.join(",")})`, `tuple(${models.info.types.join(",")})`];
  try {
...
export function isValidPosition(data, position) {
  ...
  return (
    position.uniqueOwner !== VOID_ETHEREUM_ADDRESS &&
    position.creationBlock !== "0" &&
    position.uniqueOwner.toLowerCase() === account.toLowerCase()
  );
}
...
export async function loadFarmingPosition(data, farmingContract, positionId) {
  ...
  try {
    ...
    var uniswapV3NonfungiblePositionManager = newContract(
    ...
    position.liquidityPoolTokenAmount = await blockchainCall(
      uniswapV3NonfungiblePositionManager.methods.positions,
      position.tokenId,
    );
...
export async function addLiquidityGen2(data) {
  ...
  if (prestoData) {
    ...
    sendingOptions.gasLimit = parseInt(
      sendingOptions.gasLimit * (getNetworkElement({ context, chainId }, "farmGasMultiplier") || 1),
    );
...
export async function getFarmingSetupInfo(data, element) {
  ...
  list = await Promise.all(
    list.map(async it => {
      ...
      const liquidityPoolToken = await findLiquidityPoolToken(
        data,
        element.generation,
        it.liquidityPoolTokenAddress,
        gen2SetupType,
      );
...
export function findLiquidityPoolToken(data, generation, address, gen2SetupType) {
  return address && isEthereumAddress(address)
    ? generation === "gen1"
      ? findLiquidityPoolTokenGen1(data, address)
      : findLiquidityPoolTokenGen2(data, address, gen2SetupType)
    : undefined;
}
...
export async function findLiquidityPoolTokenGen2(data, address, gen2SetupType) {
  ...
  const lpInfo = [
    [],
    [],
    [await blockchainCall(poolContract.methods.token0), await blockchainCall(poolContract.methods.token1)],
  ];
  const ammData = [
  ...
  await Promise.all(
    lpInfo[2].map(async tkAddress => {
      ...
      const symbol =
        tkAddress === VOID_ETHEREUM_ADDRESS || tkAddress === ammData[0]
          ? "ETH"
          : await blockchainCall(currentToken.methods.symbol);
      var name =
        tkAddress === VOID_ETHEREUM_ADDRESS || tkAddress === ammData[0]
          ? "Ethereum"
          : await blockchainCall(currentToken.methods.name);
      var decimals = parseInt(
        tkAddress === VOID_ETHEREUM_ADDRESS ? "18" : await blockchainCall(currentToken.methods.decimals),
      );
      tokens.push({
        symbol,
        name,
        decimals,
        address: tkAddress,
        isEth: tkAddress.toLowerCase() === ammData[0].toLowerCase(),
      });
